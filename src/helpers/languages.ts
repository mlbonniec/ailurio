import type { Languages, LanguagesColor } from 'types/github';

const colors: LanguagesColor = {
...
function ratio(languages: Languages): { [k: string]: number;} {
  ...
  const clean = ofe(oe(languages).filter((e): e is [string, number] => e[1] !== undefined));
	const total = ov(clean).reduce((previous, current) => previous + current);

  // Set languages ratio to 100
  oe(clean).forEach(([language, ratio]) => clean[language] = ratio/total*100);
  
  return clean;
}

export default function getLanguages(languages: Languages) {
	// Convert ratio to percent
  const percentages = ratio(languages);

	return Object.keys(percentages).map(e => {
		const language = e as keyof Languages;

		return { name: language, ratio: percentages[language], color: colors[language]};
	}).sort((a, b) => {
